...
- `python -m pytest --collect-only -q`: 311 tests collected.
- `python -m pytest -q`: 288 passed, 23 failed, 20 warnings.
- Main failing area: survival/censoring code, matching/censoring propagation, report output, and yearly backtesting first-year handling.
- Worktree was already dirty before this audit. Do not overwrite existing agent-batch/code changes without reviewing them.
...
- Full pipeline can be rerun from raw/local data without manual intervention.
- Artifact manifest matches files on disk.
- README, METHODOLOGY, RESULTS, ROADMAP, dashboard copy, and generated reports use the same target definitions.
- No final-facing doc says "TODO", "PARTIAL", "stub", or stale agent-batch status unless it is explicitly in `docs/internal` or `docs/old`.
- Dashboard never shows fake default predictions as real model output.
- Survival modeling is either genuinely working or explicitly future work everywhere.
- LOS and adopted-only timing are never conflated.

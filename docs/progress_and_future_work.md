...
- single-animal or what-if prediction page.
...
The next sprint should be narrow and output-focused.

Recommended sprint goal:

> Turn the existing metrics and hypothesis tables into thesis-ready report artifacts.

Recommended tasks:

1. Add a reporting module and script.
2. Generate Markdown summary from existing CSVs.
3. Generate classification and regression comparison plots.
4. Generate H1/H3/H5 plots.
5. Add tests for report-generation outputs.
6. Update README with the new command.
...
This keeps the project moving toward thesis deliverables without destabilizing the working ML pipeline.
...
Build Streamlit only after stable report outputs exist. The app should present results, not become the main analysis engine.
...
This confirms that the current data-preparation, feature-engineering, modeling-output, and analysis-output tests pass in the local environment.

...
Na podstawie wyników z okresu selekcyjnego 2023 wybrano dwa różne modele końcowe: CatBoost dla klasyfikacji adopcji oraz Random Forest dla regresji długości pobytu. Zbiór testowy z lat 2024--2025 wykorzystano wyłącznie do jednorazowej oceny ich zdolności generalizacji.

Różnica między zadaniami jest dobrze widoczna w metrykach. Wybrany klasyfikator osiągnął ROC-AUC około 0,841, natomiast wybrany model regresyjny uzyskał MAE 20,07 dnia. Warto też zwrócić uwagę na gatunek: wyniki dla kotów są lepsze niż dla psów. Może to oznaczać, że ścieżki pobytu kotów są w badanym materiale bardziej powtarzalne albo mniej zależne od nietypowych przypadków.
...
\begin{table}[htbp]
\centering
\caption{Wyniki modeli wybranych na walidacji, ocenionych na zbiorze testowym (2024--2025)}
...
\hline
\rowcolor{LightGray}
\textbf{Zadanie} & \textbf{Podzbiór danych} & \textbf{Wybrany model} & \textbf{Główna miara (ROC-AUC / MAE)} & \textbf{PR-AUC} & \textbf{F1-Score} & \textbf{Precision} & \textbf{Recall} & \textbf{RMSE} & \textbf{MedAE} \\
\hline
Klasyfikacja & Zbiór łączny & CatBoost & \textbf{0,841} (ROC-AUC) & 0,885 & 0,817 & 0,838 & 0,797 & -- & -- \\
\hline
Klasyfikacja & Psy & CatBoost & \textbf{0,810} (ROC-AUC) & 0,858 & 0,798 & 0,820 & 0,777 & -- & -- \\
\hline
Klasyfikacja & Koty & CatBoost & \textbf{0,868} (ROC-AUC) & 0,902 & 0,849 & 0,840 & 0,859 & -- & -- \\
\hline
Regresja & Zbiór łączny & Random Forest & \textbf{20,07} (MAE dni) & -- & -- & -- & -- & 35,82 & 10,82 \\
\hline
Regresja & Psy & Random Forest & \textbf{22,22} (MAE dni) & -- & -- & -- & -- & 41,95 & 9,84 \\
\hline
Regresja & Koty & Random Forest & \textbf{18,16} (MAE dni) & -- & -- & -- & -- & 29,27 & 11,36 \\
\hline
\end{tabular}%
}
\end{table}
...
\caption{Wartości rzeczywiste kontra przewidywane dla wybranego modelu regresyjnego (Random Forest). Widoczny jest systematyczny niedoszacunek dla długich pobytów (powyżej 60~dni).}
...
\caption{Rozkład reszt modelu regresyjnego. Silna asymetria prawostronna potwierdza koncentrację błędów na rzadkich przypadkach bardzo długich pobytów -- MAE wynosi 20{,}07~dnia, natomiast mediana błędu bezwzględnego 10{,}82~dnia.}
...
\ChapterFiveFigure{reports/figures/diagnostic_roc_curve.png}{Szczegółowy przebieg krzywej ROC dla wybranego klasyfikatora (CatBoost). Widoczne wyraźne wysklepienie nad linią losową (AUC = 0.50).}{fig:diagnostic-roc-curve}
...
W zadaniu regresji szacującym liczbę dni do wyjścia ze schroniska model Random Forest wybrano na podstawie najniższego błędu MAE w okresie selekcyjnym 2023. Do oceny tego zadania wykorzystano metryki opierające się na różnicy między predykcją a wartością rzeczywistą: średni błąd bezwzględny (MAE) oraz pierwiastek błędu średniokwadratowego (RMSE). Rysunki \ref{fig:model-comparison-regression-mae} i \ref{fig:model-comparison-regression-rmse} pokazują graficzne zestawienie błędów dla poszczególnych algorytmów.
...
\ChapterFiveFigure{reports/figures/model_comparison_regression_rmse.png}{Zestawienie błędu RMSE dla modeli regresyjnych. Metryka silniej waży duże odchylenia i uzupełnia kryterium MAE użyte do wyboru modelu.}{fig:model-comparison-regression-rmse}

Modele nieliniowe poradziły sobie z tym wyzwaniem zauważalnie lepiej od średniej czy mediany bazowej. Algorytm ten jest w stanie optymalnie wyłapać nieliniowe relacje w strukturze danych wejściowych, co ma kluczowe znaczenie przy zjawiskach z dużą liczbą wartości odstających.
...
Trzeba przy tym uczciwie zaznaczyć jedną decyzję metodologiczną. Wybranym klasyfikatorem końcowym jest CatBoost, dla którego wygenerowano wykresy SHAP przy użyciu \emph{TreeExplainer}. Interpretacje służą do pokazania kierunków zależności predykcyjnych i nie stanowią dowodu przyczynowości.

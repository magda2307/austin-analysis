...
- For every drafting, rewriting, proofreading, translating, or style-review task
  involving Polish thesis prose, use `$polish-thesis-human-style` from the
  project-local `.codex/skills` directory.
- Never invent, infer, omit, merge, or silently correct facts, values, dates,
  scopes, caveats, citations, target definitions, or degrees of certainty during
  stylistic editing. When equivalence is uncertain, retain the original wording
  and flag the issue.
...
- Keep the validated selection conclusion intact: CatBoost is selected for classification using 2023 validation evidence and reaches test ROC-AUC around `0,841`.
- Keep the validated selection conclusion intact: Random Forest is selected for regression using 2023 validation evidence and reaches test MAE around `20,07` days.

...
Zbiór 162 355 epizodów podzielono chronologicznie na trzy części:

\begin{itemize}\itemsep2pt
\item \textbf{Zbiór treningowy:} lata 2013--2021. Pula obejmująca 126 731 rekordów psów i kotów.
\item \textbf{Zbiory walidacyjne:} rok 2022 obejmuje 11 153 epizody i służy do kalibracji oraz doboru progów, a rok 2023 obejmuje 10 654 epizody i służy do wyboru modeli. Łącznie oba zbiory obejmują 21 807 epizodów.
\item \textbf{Zbiór testowy:} lata 2024--2025. Obejmuje 13 817 epizodów. Służy wyłącznie do jednorazowej oceny zamrożonych, wcześniej wybranych modeli na najnowszych danych.
\end{itemize}
...
\node[box] (train) at (0,0) {Zbiór treningowy\\2013--2021\\126\,731 rekordów};
\node[box] (val) at (5.5,0) {Kalibracja i wybór\\2022--2023\\21\,807 rekordów};
\node[box] (test) at (11,0) {Zbiór testowy\\2024--2025\\13\,817 rekordów};
...
$D_{cal} \gets$ podzbiór $D$ (rok 2022)\;
$D_{sel} \gets$ podzbiór $D$ (rok 2023)\;
...
\ForEach{zadanie $\in \{ \text{klasyfikacja}, \text{regresja} \}$}{
  \ForEach{algorytm $\in \{ \text{Dummy, Logistic/Ridge, RF, HistGB, CatBoost} \}$}{
    ...
    Trenuj $M_{alg}$ na $D_{train}$ używając \texttt{early\_stopping} na $D_{cal}$\;
    \If{zadanie == \text{klasyfikacja}}{
      Skalibruj prawdopodobieństwa i zoptymalizuj próg F1 na $D_{cal}$\;
    }
    $\hat{y}_{sel} \gets$ Prognozuj dla $D_{sel}$ z modelem $M_{alg}$\;
    $E_{alg}^{sel} \gets$ Oblicz metryki wyboru($\hat{y}_{sel}$, $D_{sel}$)\;
  }
  $M^* \gets$ Wybierz najlepszy model na podstawie $E^{sel}$ i zamroź jego konfigurację\;
  $\hat{y}_{test} \gets$ Prognozuj dla $D_{test}$ z modelem $M^*$\;
  $E^{test} \gets$ Oblicz końcowe metryki($\hat{y}_{test}$, $D_{test}$)\;
  Zapisz $M^*$ (\texttt{.joblib}) oraz $E^{test}$ (\texttt{.csv})\;
}
Wyprowadź globalne i lokalne wartości SHAP dla wybranych modeli\;
\end{algorithm}
